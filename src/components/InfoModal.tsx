...
import { Info, X, Zap, Activity, TrendingDown, Target, Brain, Layers, RefreshCw, BarChart3, Database, Code } from 'lucide-react';
        ...
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md">
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto pr-6 space-y-16 custom-scrollbar pb-10">
              ...
              {/* SECTION 11: PYTORCH EXTENSION & CNN SUPPORT */}
              <section>
                <div className="flex items-center gap-3 mb-8">
                  <Code className="w-6 h-6 text-emerald-600" />
                  <h3 className="text-xl font-bold text-[#1C1917] uppercase tracking-wider">11. PyTorch Extension & CNN Support</h3>
                </div>
                <div className="space-y-6 text-[#44403C] leading-relaxed">
                  <p>
                    For research-level experimentation, Neur-O-Opt Lab includes a <strong>PyTorch Extension</strong> (`optimizer_comparison_pytorch.py`) that supports Convolutional Neural Networks (CNNs).
                  </p>
                  
                  <div className="grid md:grid-cols-2 gap-8">
                    <div className="space-y-3">
                      <h4 className="font-bold text-[#1C1917]">CNN Architectures</h4>
                      <ul className="space-y-2 text-sm">
                        <li className="flex gap-2">
                          <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 mt-1.5 shrink-0" />
                          <span><strong>SimpleCNN:</strong> Optimized for MNIST/Fashion-MNIST with 2 conv layers and BatchNorm.</span>
                        </li>
                        <li className="flex gap-2">
                          <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 mt-1.5 shrink-0" />
                          <span><strong>DeepCNN:</strong> 3-block architecture for CIFAR-10 with Dropout and BatchNorm for stability.</span>
                        </li>
                      </ul>
                    </div>
                    
                    <div className="space-y-3">
                      <h4 className="font-bold text-[#1C1917]">Advanced Metrics</h4>
                      <ul className="space-y-2 text-sm">
                        <li className="flex gap-2">
                          <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 mt-1.5 shrink-0" />
                          <span><strong>Layer-wise Grad Flow:</strong> Track gradient norms for every layer individually.</span>
                        </li>
                        <li className="flex gap-2">
                          <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 mt-1.5 shrink-0" />
                          <span><strong>Update Efficiency:</strong> Measure the ratio of parameter updates to parameter norms.</span>
                        </li>
                      </ul>
                    </div>
                  </div>
                  ...
                  <div className="bg-emerald-50 border-l-4 border-emerald-500 p-6 rounded-r-2xl">
                    <p className="text-emerald-900 text-sm italic">
                      "The PyTorch extension allows for a fair comparison of optimizers across both MLP and CNN architectures, providing deeper insights into how different algorithms handle spatial features and deep hierarchies."
                    </p>
                  </div>
                </div>
              </section>
